...
export default {
    ...
    async created() {
        ...
        await axios
            .get(`/api/get-kho-hang-id/${this.id}`)
            .then((result) => {
                this.dataResposed = result.data.data[0];
                this.ten_kho = this.dataResposed.tls_ten_kho;
            })
            .then((result) => {
                this.loading = false;
            })
    ...
    methods: {
        async updateDataKho() {
            ...
            await axios
                .post("/api/update-kho-hang", {
                    tls_id_kho: this.id,
                    tls_ten_kho: this.ten_kho,
                })
                .then((result) => {
                    if (result.data.status_code == 200) {
                        alert("Chúc mừng bạn đã cập nhật kho thành công.");
